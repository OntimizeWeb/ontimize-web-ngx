...
* **o-form-toolbar**: The confirm/accept button now shows the label matching the form's actual mode instead of always reading `'INSERT'`. It is keyed by `attr='insert'` in INSERT mode and `attr='update'` in UPDATE mode, but the template resolved its label from a hardcoded literal regardless of mode; it now reads it through `getActionLabel(acceptButtonAttr)` like every other built-in button, so it shows `'ADD'` in INSERT mode and `'SAVE'` in UPDATE mode.
