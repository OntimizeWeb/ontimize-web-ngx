...
* **o-table / o-form**: `O_TABLE_GLOBAL_CONFIG` and `O_FORM_GLOBAL_CONFIG` now also accept an `Observable<OTableGlobalConfig>` / `Observable<OFormGlobalConfig>`, in addition to a plain value. A plain config still applies once, at each component's construction, as before; providing an Observable instead re-applies the config on every emission to every already-constructed `o-table` / `o-form`, so an app can flip a global default (e.g. `showButtonsText`) at runtime and have it take effect everywhere without recreating any component. Backwards compatible — existing `useValue: {...}` providers are unaffected.
### Bug Fixes
* **o-app-sidenav**: `id` (in addition to the existing `data-testid`) is now set on `o-app-sidenav-menu-item`'s collapsed/rail-state elements, and on `o-app-sidenav-menu-group`'s root element in both open and collapsed states — none of them ever had `id`, only `data-testid`. `data-testid` was documented (`18.0.0-next.13`) as reusing each item's existing `id`, but that premise only held for `menu-item`'s open state; everywhere else `id` was simply never set on that element.

## 18.0.0-next.15 (2026-08-24)
### Bug Fixes
* **o-table**: `show-buttons-text` (and the underlying `showButtonsText`) had no effect when changed after the table's initial render — only a value already set at mount time actually applied. `o-table-button` and its wrapper `o-table-buttons` (which renders the built-in insert/refresh/delete buttons) are both `ChangeDetectionStrategy.OnPush`, and `o-table-button`'s template read `table.showButtonsText` directly off the injected table instance rather than from one of its own `@Input`s — under `OnPush`, Angular only re-checks a component when one of its own inputs changes, an event fires from within it, or it's explicitly marked dirty, none of which happens when only the table's `show-buttons-text` changes elsewhere. `OTableComponent` now exposes a `showButtonsTextChange` observable alongside the existing `showButtonsText`; `o-table-button` subscribes to it and calls `markForCheck()` on every emission, so both the built-in buttons and any custom `o-table-button` projected into `<o-table>` now correctly switch between text and icon-only when toggled at runtime.
* **o-date-input**: `getValue()` no longer special-cases a `Date` instance into a timestamp. That conversion (added in `18.0.0-next.14`, `1b5cb16c`) ran unconditionally whenever the stored value happened to be a native `Date`, ignoring `value-type` entirely — so e.g. `value-type="date"` silently got back a raw timestamp number instead of the Date-like value callers expect for that type. `ODateInputComponent` no longer overrides `getValue()` at all, falling through to `OFormDataComponent`'s own passthrough (return whatever is stored, untouched) — consistent with what `hasComponentChanged()` already compares against, and with every other `value-type` (string/timestamp/ISO), which were never coerced.

## 18.0.0-next.14 (2026-08-20)
### Bug Fixes
* **o-date-input**: Clearing a date field and saving could wrongly show "nothing to update". Cause: the form's change-detection cache is populated via each field's `getValue()`, but the live "did it change" check compares against the raw `FormControl.value` instead — normalized vs. raw is a fragile comparison for a type with as many representations as a date. `getValue()` also returned `undefined` for a backend-supplied date **string** under `value-type="date"` (only a `Date` instance was ever recognized), and extending it to accept strings there turned out to corrupt the datepicker's own displayed text (it reuses the same normalization). Fix: `getValue()` is reverted to its pre-`c7818c51` behaviour — return the raw stored value as-is, only converting a `Date` instance to a timestamp — matching what the change-detection check already compares against.
* **o-data-toolbar**: Now wraps onto extra lines (`flex-wrap: wrap` + `gap: 8px`) instead of overflowing a single row when its content — title, quickfilter, buttons, menu — doesn't fit. `o-table`/`o-list`/`o-grid`'s toolbar (`.o-data-toolbar`, `display: flex; flex-direction: row`) had no `flex-wrap`, so on narrow screens the excess just overflowed the toolbar's own box; if an ancestor happened to clip overflow (e.g. an app wrapping the table in a rounded card via `overflow: hidden`), part of the toolbar — potentially an entire button or the options menu — became invisible and unreachable, with no scrollbar to get to it either. The `gap` avoids wrapped items (e.g. the quickfilter and the buttons) touching each other with no spacing, both within a wrapped row and between wrapped lines.
* **o-form**: `.fill-form` used `height: 100%`, which always claims its full parent's height regardless of any sibling also taking up space in the same `flex-direction: column` container (e.g. a page header sitting above the form) — the combined demand silently overflowed the parent by exactly the sibling's height. Now also sets `flex: 1; min-height: 0;`: when the parent is a flex container, `flex-basis: 0%` (from `flex: 1`) takes precedence over `height` for sizing, so `<o-form class="fill-form">` fills whatever space is actually left instead of an unconditional 100%; `height: 100%` remains as the fallback for non-flex parents, where `flex` has no effect. This was the last piece of a chain of very similar `min-height: auto`/"always claims 100%, ignores siblings" bugs found today across `.o-flex-grow` (this same file's `.o-flex-grow` sibling utility, `flex-layout.scss`) and an app's own page-chrome CSS — each one only became visible once a table without virtual scroll (e.g. grouped) had enough real content to expose the slack.
* **o-table**: Grouping (or ungrouping) a column could leave the loading skeleton stuck forever. Changing the grouped columns flips `enabledVirtualScroll`, which recreates the whole `<table>` — and with it, a brand-new `DefaultOTableDataSource` — on the next change-detection pass; the pending `groupByColumnChange` emission that was supposed to trigger `executeDataProcessing` and clear the skeleton got silently dropped when the old `dataSource.connect()` subscription tore down before that (`observeOn(asyncScheduler)`-delayed) event was delivered. `connect()`'s own bootstrap guard (skip processing when nothing has been queried yet) couldn't help either: it keyed off the new `DefaultOTableDataSource` instance's own `renderedData`, always `null` right after any rebuild, so it couldn't tell a genuinely-never-queried table apart from one being rebuilt with real data already sitting in `_database`. `OTableDao` now tracks `hasLoadedOnce` (survives rebuilds, unlike `renderedData`), so `connect()` correctly runs a full processing pass on the fresh `dataSource`'s first `dataChange` replay whenever the table already had data — which reliably fires because `dataChange` is a `BehaviorSubject`, replayed to whoever subscribes regardless of timing, unlike the plain `Subject` `groupByColumnChange` emission that could be missed.
* **theming**: `.o-flex-grow` (`flex-layout.scss`) now sets `min-height: 0; min-width: 0;`, matching the sibling `.o-flex` utility. Flex items default to `min-height`/`min-width: auto` — never shrink below their content's natural size — and that auto value only resolves to 0 when the item's own `overflow` isn't `visible`; `.o-flex-grow` set neither, so a `.o-flex-grow` wrapping an unbounded child (e.g. a table) inside a `flex-direction: column` ancestor ignored the space that ancestor actually had and grew to fit the child instead. Concretely: with `o-table`'s `fixed-header="true"`, grouping a column disables virtual scroll ([o-table.component.ts:3710](projects/ontimize-web-ngx/src/lib/components/table/o-table.component.ts#L3710)) and switches from `cdk-virtual-scroll-viewport` (which only ever renders its visible row window, so it never demanded much height) to rendering every row — and if the app's own layout happens to wrap the table in a `.o-flex-grow`, that's what stopped the table from respecting its container's height once it had all those real rows to size against.
* **o-table**: Toggling virtual scroll on/off at runtime (e.g. dynamically enabling/disabling `groupable`, or the `virtual-scroll` input itself) while a sort was active reset the sort arrow indicators on the column headers, even though the data stayed correctly sorted. The `@if`/`@else` switch that swaps between the virtual-scroll and plain-scroll templates recreates the whole `<table>`, including a brand-new `OMatSort` instance with no active sort — `registerSortListener()` only re-subscribed to its change events and forgot to restore `sortColArray` onto it. Compounding this, `registerTableHeaders()` skipped registering a header when one with the same `attr` was already present, so after the rebuild it kept referencing the old, destroyed header instances instead of the new ones, meaning even an explicit `refreshSortHeaders()` call would have refreshed nothing. Both are fixed: the current sort is now reapplied to a freshly-connected `OMatSort`, and re-registering a header now replaces its stale entry instead of being dropped.
* **o-table**: Sorting or paginating a `pageable` table stopped showing the loading skeleton and instead flashed the previous, stale rows before the new ones appeared. `queryData()` hid the skeleton (by forwarding `loadingSubject`'s `false` to `OTableLoadingService`) as soon as the HTTP response arrived, but the table's own pipeline that turns that response into rendered rows (`executeDataProcessing`: filtering/sorting/pagination-slicing/virtual-scroll windowing) runs asynchronously afterwards — so the skeleton always disappeared before the rows it was covering were actually ready to paint. `executeDataProcessing`'s own `setLoading(false)` call, meant to be the real "done" signal, always lost that race and was dead code in practice. A new `awaitingTableRender` flag now suppresses the premature signal for the duration of a query, letting `executeDataProcessing` be the one that actually hides the skeleton; it falls back to the previous immediate behaviour when the query can't reach the data service at all (missing `dataService`/`entity`), so a misconfigured table doesn't get stuck showing the skeleton forever. Client-side sort/pagination on a non-`pageable` table was not affected by this race (it never goes through `loadingSubject`) and keeps working as before.
* **o-table**: `onChangePage` no longer passes a Material `PageEvent` to `OTableLoadingService.handleProtected()`. That method expects a `MouseEvent` and calls `preventDefault()`/`stopPropagation()` on it when a click is blocked (e.g. a second page change while a previous one is still loading) — `PageEvent` has neither method, so a blocked page change threw a `TypeError` instead of being silently ignored.

## 18.0.0-next.13 (2026-08-06)
### Features
* **testing**: New `data-testid` input, for E2E testing (Playwright/Cypress), forwarded to the actual native element the user interacts with — never to the Angular component's own host tag, unless the host *is* that element. Optional, no effect on existing behaviour when unset.
  * **Form inputs** (`o-text-input`, `o-integer-input`, `o-real-input`, `o-percent-input`, `o-currency-input`, `o-email-input`, `o-phone-input`, `o-nif-input`, `o-password-input`, `o-textarea-input`, `o-date-input`, `o-daterange-input`, `o-hour-input`, `o-checkbox`, `o-slide-toggle`, `o-slider`, `o-combo`, `o-radio`, `o-listpicker`, `o-html-input`, `o-image`, `o-file-input`, `o-button-toggle`): new `data-testid` defined once on the shared `DEFAULT_INPUTS_O_FORM_DATA_COMPONENT` (`OFormDataComponent`), so every one of them gets it without per-component duplication, mirroring how `[id]="getAttribute()"` already reaches the same native element today. `o-file-input` exposes two ids off the same value — `data-testid` on the visible field, `${data-testid}-file-input` on the hidden native `<input type="file">` — mirroring its existing `id`/`id + '-file-input'` split. `o-listpicker` and `o-radio` route it to their real interactive element (the visible search input / the `mat-radio-group`) rather than the hidden proxy `<input>` used only for label/error display.
  * **o-button**: new `data-testid`, applied to whichever native `<button>` variant actually renders (basic/raised/stroked/flat/icon/fab/mini-fab).
  * **o-table / o-list / o-grid / o-tree**: new shared `data-testid` on `AbstractOServiceBaseComponent`, used as a *prefix* for the built-in controls rather than applied verbatim, since there is no single native element at that level: the quick-filter input gets `${data-testid}-quick-filter` (`o-table`'s own `OTableQuickfilterComponent`; `o-search-input`, reused by `o-list`/`o-grid`/`o-tree`, also gained its own `data-testid` input for this), and the insert/refresh/delete buttons get `${data-testid}-insert` / `-refresh` / `-delete` — via `o-table-button`'s own `dataTestId` getter for `o-table`, and via a new shared `getDataTestId(attr)` helper on `AbstractOServiceComponent` for `o-list` / `o-grid`'s own inline buttons (text, icon-only and floating-FAB variants alike).
  * **o-app-sidenav**: the toggle icon (`sidenav-menu`), every menu item/group (`o-app-sidenav-menu-item` / `-menu-group`, both open and closed/rail state) keyed by the item's own `menuItem.id` / `menuGroup.id` (already used for `id` — same value, same duplicate-if-app-reuses-it caveat), and the group's expand/collapse arrow icon.
  * **o-user-info**: the menu trigger (`user-info-trigger`) and the settings/logout menu items (`menu-settings` / `menu-logout`).
  * **o-bar-menu**: new `data-testid` input, independent of its internal `id` (always an auto-generated random value, not consumer-settable) — applied to the toggle icon. Its menu items (`o-bar-menu-group`, `o-bar-menu-item`, `o-locale-bar-menu-item`) gained the same input via a new shared `DEFAULT_INPUTS_O_BASE_MENU_ITEM` entry on `OBaseMenuItemClass`.
  * **Dialogs**: the shared alert/confirm dialogs (`o-dialog`, internal variant) and all 13 built-in dialogs (table export/apply-configuration/store-configuration/visible-columns/group-by-columns/filter-by-column, load/store-filter, list-picker, fullscreen image, form-layout close/fullscreen) now expose their action buttons via static `data-testid`s (`dialog-cancel` / `dialog-accept` / `dialog-ok` / `dialog-delete` / `dialog-clear`), since only one modal dialog is normally open at a time.
  * **Per-row content**: new `getRowDataTestId(row)` on `AbstractOServiceComponent`, composing this component's `data-testid` with the row's key value(s) (via the existing `extractKeysFromRecord`) — `${data-testid}-row-${keyValues.join('-')}`, or `null` when either isn't set. `o-table` wires it directly onto its own `<tr>` rows, the selection checkbox (`-checkbox` suffix) and the built-in `editButtonInRow`/`detailButtonInRow` cell (`-editButtonInRow` / `-detailButtonInRow` suffix). `o-list` / `o-grid` don't render row markup themselves (it's projected from the consumer's own template via `<ng-content>` / `item.template`), so there's nothing for the framework to wire there — `getRowDataTestId(row)` is public precisely so a consumer's own row template can call it directly (e.g. `[attr.data-testid]="pendingReviewsList.getRowDataTestId(row)"`).
  * **Table cell editors** (text/integer/real/email/date/time/boolean): new `dataTestId` getter on `OBaseTableCellEditor`, composing `getRowDataTestId()` with the column's `attr` — independent of the existing `cellEditorId`, which stays a `Util.randomNumber()`-based internal id (used for `document.getElementById` focus lookups) and was never meant to be a stable test selector.
  * **`o-radio`**: each `mat-radio-button` option now also gets `${data-testid}-option-${index}`, independent of its `id` (which stays `groupId`-based — `groupId` is a `crypto.randomUUID()` regenerated on every component instance, so unlike the option's index it was never usable as a stable test selector).
  * **Fallback to `attr`**: `data-testid` now falls back to the component's own `attr` when not explicitly set — on every component built on `OFormDataComponent` (form inputs), `AbstractOServiceBaseComponent` (`o-table`/`o-list`/`o-grid`/`o-tree`), `o-button`, `o-button-toggle` and `OBaseMenuItemClass` (`o-bar-menu`'s menu items). `attr` is already a stable, meaningful identifier on these components (it's what `[id]="getAttribute()"` already uses), so most components now get a sensible test id for free without setting both `attr` and `data-testid`. Implemented as a getter/setter pair (`dataTestId ?? attr`), so it composes transparently with `getDataTestId(attr)` / `getRowDataTestId(row)`. `o-bar-menu` itself and `o-search-input` have no `attr` input, so they keep no fallback (stay `undefined` when unset).
### Bug Fixes
* **o-form**: Removed `stayInRecordAfterEdit = stayInRecordAfterEdit || isEditableDetail()` from `determinateFormMode()`. Since `editable-detail` defaults to `true`, this silently forced `stayInRecordAfterEdit` to `true` on virtually every form (regardless of the explicit `stay-in-record-after-edit` input, which defaults to `false` and is asserted as such in `o-form.component.spec.ts`), making `update()` skip `closeDetail()` and never navigate back — the two concerns (staying in a directly-editable detail view vs. explicitly opting into "stay after edit") were unrelated and shouldn't have been coupled.
* **o-form**: `update()`'s `closeDetail()` call now passes `{ exitWithoutConfirmation: true }`, matching the existing `insert()` (`_clearAndCloseFormAfterInsert`) and `deleteData()` (`canDiscardChanges = true`) success paths. Previously, closing the detail right after a *successful* update could still trigger the "discard unsaved changes?" confirmation dialog.

## 18.0.0-next.12
### Features
* **o-table / o-list / o-grid**: New shared input `no-results-message`, defined on `AbstractOServiceComponent`, to override the default "no results" empty-state message (`TABLE.EMPTY` on `o-table` / `o-list`, `GRID.EMPTY` on `o-grid`). Accepts plain text, an i18n key (resolved with the translate service), or an HTML string — sanitized before being rendered, so markup like `<b>`/`<br>`/`<a>` is allowed while `<script>`/event-handler attributes are stripped. `o-list` / `o-grid` bind it through `[innerHTML]` (sanitized automatically by Angular); `o-table`'s message row is built imperatively by `OTableExpandedFooterDirective`, which now sanitizes it explicitly via `DomSanitizer.sanitize(SecurityContext.HTML, ...)` before assigning `innerHTML`. When set, it replaces the whole default message (including the `*_USING_FILTER` suffix).
* **o-table / o-list / o-grid**: New shared content-projection slot `<ng-template #noResultsTemplate>`, also defined on `AbstractOServiceComponent` (mirrors `o-tree`'s `leafNodeTemplate` / `parentNodeTemplate`), for a fully custom empty-state — e.g. an icon alongside the text — which `no-results-message` alone can't render since Angular's sanitizer strips non-standard elements like `<mat-icon>` from HTML strings. Takes precedence over `no-results-message`, which in turn takes precedence over the default translated message. `o-list` / `o-grid` render it declaratively via `*ngTemplateOutlet`; `o-table`'s `OTableExpandedFooterDirective` creates the embedded view through its `ViewContainerRef` and relocates its root nodes into the message `<td>` (the same technique the CDK's `TemplatePortal` uses), since that row is built imperatively rather than through the component's own template.

## 18.0.0-next.11
### Bug Fixes
* **o-button**: Fixed the icon/label spacing when `icon-position="right"`. Material's own icon margin (`.mat-mdc-button > .mat-icon`) always assumes a leading icon — `margin-right` for the gap, `margin-left: 0` — so with the icon placed after the label that gap ended up on the wrong side and the icon rendered touching/overlapping the label. A new `o-button-icon-position-right` host class (mirroring the existing `-top` / `-bottom` classes, previously missing for `right`) mirrors the margins for this position only (`margin-left: var(--mat-text-button-icon-spacing, 8px)`, `margin-right: 0`).
* **o-list**: Removed a stray `background-color` (`--o-bg-level-08`) and `border-radius` on `.o-list-skeleton-item`. Real `mat-list-item`s render with a transparent background, but the skeleton placeholder gave each row its own darker card-like background while its inner loader bars used the much lighter `--o-bg-level-04` — the bars rendered lighter than their own container, an inverted, mismatched look not shared by `o-table` / `o-grid`'s skeletons (which stay transparent). The list skeleton now matches the others.

## 18.0.0-next.10
* **o-table / o-list / o-grid**: The `row-height` input (`small | medium | large`) no longer has any visual effect, and neither does the legacy `dense` attribute of `mat-list` / `mat-selection-list` (dropped by Material's MDC-based components). Row heights are now driven by the theme's **density**: set the `density` parameter of the theme factory (`o-mat-light-theme((density: -4))`) for the whole app, apply `ontimize-theme-density-extended(<scale>)` to a specific scope, or use Material's per-component density mixins (e.g. `mat.list-density(-3)`). The `row-height` input remains in the API so existing templates keep compiling, but it is inert and should be removed. See the migration guide (section 3.5).

### Bug Fixes
* **o-app-sidenav**: Nested menu items (open state) now render their label text a size smaller than the root items, giving the level-1/level-2 hierarchy a visual cue beyond row height and indentation. `--mdc-list-list-item-label-text-size` is set to `var(--mat-sys-body-medium-size)` (0.875rem) for both nesting levels — level-1 and level-2 share this step, since Material's M3 type scale has no distinct token between it and the root's `body-large`; the two levels remain visually distinct via row height (36px / 32px), icon size (24px / 20px) and indentation. Being `rem`-relative, it also scales with the density-driven root font-size (14px at density 0/-1/-2, ~12px at -3/-4/-5) instead of a fixed pixel value. Previously both levels inherited Material's `body-large` list-item default — the same size as the root items, with no text-size distinction at all. Also added `padding-right: 8px` to every open-state nav-list item.
* **o-form-toolbar**: The confirm/accept button now consistently shows a save icon and the `'SAVE'` label in **both** INSERT and UPDATE mode, instead of a hardcoded checkmark icon with a label that always read `'INSERT'` regardless of mode. The button's `attr` HTML attribute still switches between `insert` / `update` per mode (used for CSS/e2e hooks and to resolve its importance/colour, `primary` in both modes via the component's auto-rules) — only the icon and visible text are now unified to `getActionLabel('update')`, so a custom `action-styles` override on `update` (e.g. renaming `'SAVE'`) applies to this button in both modes, while one on `insert` no longer affects it.
* **o-button**: The icon (`icon` / `svg-icon` / `image`) now renders in Material's own before/after-label slot, as a sibling of `.mdc-button__label`, instead of nested inside it — matching `o-form-toolbar`'s buttons. Angular Material assigns a button's content-projection slots once, synchronously, based on what is physically present in the DOM at that moment; the previous template rendered the icon through a shared `<ng-template>` reused via `*ngTemplateOutlet`, which only exists as an empty placeholder at that moment regardless of `*ngIf`/`@if`, so the icon always fell back to the default slot inside `.mdc-button__label`. Every icon/image element is now unconditionally present in the template, with visibility toggled via `[style.display]` instead of being added/removed from the DOM, so Material can correctly bucket it. If your application's CSS specifically targeted `.mdc-button__label mat-icon` (or `.o-button-icon`/`.o-button-image` as label descendants) to style `o-button`'s icon, target it as a sibling of `.mdc-button__label` instead.

### Features
* **action-hierarchy**: `OActionStyle` (and `action-styles` / `O_ACTION_STYLES_CONFIG`) gained a new optional `label` field, so an action's **text** can be configured through the same mechanism as its `variant` / `importance` — resolved by `resolveActionStyle()` with the same precedence (instance `action-styles[attr]` → global `actions[attr]` → component auto-rule → global `default` → framework default). `o-form`, `o-form-toolbar`, `o-table`, `o-list`, `o-grid` and `o-tree` expose a new `getActionLabel(attr)` (mirroring `getActionVariant` / `getActionImportanceClass`) and now resolve every built-in button's text through it instead of a hardcoded translation key, each with its previous text as the per-`attr` auto-rule default (e.g. `o-form-toolbar`'s `undo` / `refresh` / `insert` / `edit` / `delete` / `update` / `cancel` default to `'UNDO'` / `'REFRESH'` / `'ADD'` / `'EDIT'` / `'DELETE'` / `'SAVE'` / `'CANCEL'`; `o-table` / `o-list` / `o-grid` default their create action to `'ADD'`, `o-tree`'s to `'INSERT'`), so existing apps see no change unless they opt in. Set `label` in a component's `action-styles` (a literal string or a translation key, resolved with `oTranslate`) to override one button's text, or in `O_ACTION_STYLES_CONFIG` to rename an action's text app-wide. `o-button` and `o-table-button` also resolve a new `effectiveLabel` (explicit `label` input → host/global resolved label → icon-only) when projected inside one of these components or configured through `O_ACTION_STYLES_CONFIG`, so a custom action button can likewise skip the `label` input and inherit its text from the host.
* **dates**: [Luxon](https://moment.github.io/luxon/) is now the framework's **default** date engine, replacing moment.js. New building blocks: `LuxonService` (mirrors `MomentService`'s API), `OLuxonPipe` / `oLuxon` (mirrors `OMomentPipe`), and `OntimizeLuxonDateAdapter` (extends `@angular/material-luxon-adapter`'s `LuxonDateAdapter`, mirrors `OntimizeMomentDateAdapter`). `o-date-input`, `o-daterange-input`, `o-hour-input`, `o-time-input`, `o-table-cell-editor-date` / `-time`, `o-table-filter-by-column-data-dialog` and the date/time table/combo/list-picker renderers now use Luxon internally, as does `Util.parseByValueType` and `o-table`'s year/month grouping. The default date format changed from moment's locale tokens `'L'`/`'LL'` to Luxon's equivalent macro tokens `'D'`/`'DD'` — if you pass a custom `format`/`value-format` built from moment tokens (e.g. `'DD/MM/YYYY'`), update it to Luxon's casing (`'dd/MM/yyyy'`; day and year are lowercase in Luxon, month stays `'MM'`). `DateCustomClassFunction` (the `[date-class]` input of `o-date-input` / `o-daterange-input`) now types its parameter as `any` instead of `Moment`, since it receives a Luxon `DateTime` by default or a `Moment` when the deprecated adapter is provided explicitly — existing callbacks typed against `Moment` keep compiling unchanged.
* **`custom.material.module.ts`**: `MatLuxonDateModule` / `OntimizeLuxonDateAdapter` are now the module's default `DateAdapter` / `MAT_DATE_FORMATS` wiring, replacing `MatMomentDateModule`.
* **dates**: New `O_DATE_ADAPTER` injection token (`'luxon' | 'moment'`) and `provideODateAdapter(...)` helper to choose the date adapter used by the framework's date components — app-wide (`bootstrapApplication` / `AppModule` providers) or scoped (a route's or any ancestor component's `providers`). `o-date-input`, `o-daterange-input`, `o-time-input`, `o-table-cell-editor-date` / `-time`, the filter-by-column dialog and the date/time renderers resolve their `DateAdapter` / `MAT_DATE_FORMATS` / date pipe (`oLuxon` vs `oMoment`) from it, and their internals were rewritten against the generic `DateAdapter` API so they behave identically under either adapter (default formats become `'L'`/`'LL'` under `'moment'`; `format`-like inputs are interpreted by the active adapter and `[date-class]` receives the matching date object). Not provided → Luxon.

### Deprecations
* **dates**: `MomentService`, `OMomentPipe` / `oMoment` and `OntimizeMomentDateAdapter` (with its paired `OntimizeMatDateFormats` / `dateFormatFactory`) are deprecated in favour of `LuxonService`, `OLuxonPipe` / `oLuxon` and `OntimizeLuxonDateAdapter`, but **not removed** — they keep working exactly as before and moment.js remains a real dependency for apps that provide them explicitly (e.g. `{ provide: DateAdapter, useClass: OntimizeMomentDateAdapter, deps: [MAT_DATE_LOCALE] }` alongside `dateFormatFactory`). See the migration guide (section on Luxon vs. moment.js) for how to opt back into moment.js on a raw Material datepicker.

### Breaking Changes
* **`date-range-legacy`**: The already-deprecated `o-daterange-legacy-input` component (`ODateRangeLegacyInputComponent`, `ODateRangeLegacyInputModule`, the legacy directive/config) has been removed from the framework. Use `o-daterange-input` instead.
* **icons** (retroactive note — the change itself shipped in `18.0.0-next.0`, it just wasn't called out): the Material Symbols Outlined migration shrank `assets/svg/ontimize-icon-set.svg` from ~45 icons down to 10 (`CSV`, `EXCEL`, `HTML`, `ILS`, `KRW`, `LIR`, `PDF`, `sort_by_alpha*`). Any app using `<mat-icon svgIcon="ontimize:X">` with one of the ~35 removed names (`menu`, `close`, `add`, `search`, `settings`, `delete`, `edit`, `save`, currency codes, etc.) silently renders an empty icon since `next.0`. Most removed names match a Material Symbols Outlined icon 1:1 — replace `svgIcon="ontimize:X"` with the default font icon (`<mat-icon>X</mat-icon>`). See the migration guide (section 3.2.bis) for the full list and the handful without a direct equivalent.

## 18.0.0-next.9 (2026-07-03)
### Bug Fixes
* **mat-button-toggle / o-table-export-dialog**: The selected button-toggle now shows just the checkmark instead of a filled box, and its background no longer spills past the group's rounded corners. The shared `mat-pseudo-checkbox` theme rules filled *every* checked pseudo-checkbox with a box background, which also hit the button-toggle's `appearance="minimal"` selection indicator (meant to render only a checkmark). Those box rules are now scoped to the `full` appearance (`.mat-pseudo-checkbox-full` — multi-select panels, selection lists), so the `minimal` variant falls back to Material's defaults and renders the plain checkmark. Separately, the export dialog's `.type_toggle_group` overrode the group's default `overflow: hidden` with `overflow: visible`, so the selected toggle's square background escaped the rounded border — the override was removed. So the toggles are never clipped by that `overflow: hidden` when they don't fit the dialog width, the group is now `display: flex` (filling the dialog instead of growing past it) and each toggle can shrink (`min-width: 0`), truncating long labels with an ellipsis; it also keeps its natural height (`flex-shrink: 0`) so the dialog's column layout can no longer squash it vertically when space is tight.
* **action-hierarchy / o-button**: A disabled button's **icon** is now greyed out too. The importance colour classes (`o-button--importance-*`) scoped only the label `color` to `:not(:disabled)`, but the icon colour tokens (`--mat-icon-color`, etc.) were applied unconditionally, so a disabled action kept its coloured icon. All colour overrides in `o-action-importance-color` are now scoped to `:not(:disabled)`, so a disabled button falls back to Material's disabled colours for both label and icon.
* **o-table**: The table is now transparent so it shows the parent container's background (e.g. a white card) instead of Material's opaque surface colour. The sticky header (fixed-header is on by default) keeps an opaque, theme-aware background via `var(--o-table-background, var(--o-bg-card))` so scrolled rows do not show through it; override `--o-table-background` to match a differently-coloured container.

### Features
* **o-app-sidenav**: The app-layout menu (open state) is now built on Material's `mat-nav-list` / `mat-list-item` instead of custom `ul > li > a[mat-button]` markup. Each item/group header renders as `a[mat-list-item]` with the `matListItemIcon` / `matListItemTitle` / `matListItemMeta` slots (built-in ellipsis, keyboard navigation and ARIA roles). The **appearance is unchanged**: the M3 list tokens are pinned to the legacy metrics (40/36/32px rows, 14/12/11px type, 24/20px icons, 16px icon-text gap, 8px rounded rows), the M3 hover/focus state layers are disabled in favour of the legacy `--o-sidenav-hover-bg` / `--o-sidenav-selected-bg` backgrounds and hover-primary colours (bridged into the list label/icon tokens), selection keeps using the `o-app-sidenav-viewer-sidenav-item-selected` class, and the level-1 children guide line is reproduced per item (as a background gradient at the label's old position, recolouring on hover/selection). The closed state (icon rail) keeps the previous icon-only `mat-button` markup. Apps styling the open-state menu through `.mdc-button__label` or `a.mat-mdc-button` selectors must adapt to the new list DOM.
* **theming**: `mat-button-toggle` now has the **same height as the buttons** at every density scale. `$_extended-density-tokens` documented a `--mat-standard-button-toggle-height` override that was never actually included, so the toggle followed Material's own scale (48/44/40/36/24px), never matching the button heights. The token is now emitted per scale aligned with `--mdc-*-button-container-height`: 40px at scale 0 (M3 default button height), 36px at -1/-2 and 32px at -3/-4/-5. Applies when using `ontimize-theme-density-extended()`.
* **theming**: Filled form fields now keep their floating label visible at **every** density scale. Material hides the filled label whenever the container height drops below 52px (scales -2 → -5); the theme already forced it back at -2 and now also at -3/-4/-5 via `--mat-form-field-filled-label-display: block` in `$_extended-density-tokens`, with the with-label paddings derived from Material's own formula (spec top 24 / bottom 8 minus `(56px - height) / 2`, clamped at 0): -3 → 18/2, -4 → 16/0, -5 → 14/0. Applies when using `ontimize-theme-density-extended()`.
...
* **o-table**: The `empty-cell` class is now applied based on the cell's *displayed* content instead of the raw `row[column.name]` value. A new `OColumn.isEmptyCell(row)` drives the binding: action columns (`action` / `editButtonInRow` / `detailButtonInRow`) — which render a button/icon rather than a field — and columns not bound to a data field (no `name`, or the row has no such property, e.g. custom renderers that render from row data) are never marked empty; for data-backed columns with a renderer the emptiness is evaluated over the formatted value (`renderer.getCellData`), so a value formatted to blank counts as empty and a non-blank rendering of an empty raw value does not.
* **o-date-input**: The displayed date is now cleared when the value is set to `null`/`undefined` (e.g. via `[ngModel]` or a reactive `patchValue`). `ensureODateValueType` returned early for a non-defined value without resetting the internal `dateValue`, so the `<input>` (bound to `[value]="getValueAsDate()"`) kept showing the previous date; only `clearValue()` reset it. It now resets `dateValue` on the not-defined branch too.
* **inputs**: Fixed an `ExpressionChangedAfterItHasBeenCheckedError` (NG0100) on the `subscriptSizing` binding of input components (visible on `o-date-input`). `oInputsOptions` — read by the `subscriptSizing` getter — was resolved in `ngAfterViewInit`, i.e. after the component template had already been checked, so the bound value flipped from `'fixed'` to `'dynamic'` after the first pass. It is now resolved in the constructor (alongside `selectAllOnClick`), while the DOM-dependent `Util.parseOInputsOptions` call stays in `ngAfterViewInit`.

---

## 18.0.0-next.7 (2026-06-11)
### Features
* **action-hierarchy**: New unified visual-hierarchy system for actions, decoupled from Angular Material button types. A new `OActionStyle` model (`{ variant?: 'outline' | 'flat' | 'basic' | 'raised'; importance?: 'primary' | 'warn' | 'default' }`) describes an action semantically; `importance` colours the label and icon (primary → `--mat-sys-primary`, warn → `--mat-sys-error`, default → the Ontimize foreground tokens `--o-fg-text` / `--o-fg-icon`) via the shared CSS classes `.o-action--importance-{primary,warn,default}` defined in `o-button-theme.scss`. On the shapes with a light/transparent container (`outline`, `basic`, `icon` and the elevated `raised`) the importance colours the label and icon; on the truly filled shapes (`flat`, `fab`, `mini-fab`) it instead drives the container colour through Material's palette (e.g. a `flat` + `warn` button gets a warn background), so the label keeps its legible on-container colour. Precedence is resolved by the pure `resolveActionStyle(attr, actionStyles, autoRules, default)` util: explicit configuration → component auto-rules → global default (`outline + default`).
* **o-form / o-table / o-grid / o-list / o-tree**: New optional input `action-styles` (`Record<string, OActionStyle>`, keyed by the action `attr`) to configure the appearance of the built-in actions without relying on `o-button`. By default every action renders as `outline + default`, and the **create** action is highlighted as the single primary action automatically: `o-form` makes the confirm action primary in INSERT mode (`attr=insert`) and in UPDATE mode (`attr=update`); `o-table` / `o-grid` / `o-list` / `o-tree` make the create action (`attr=add` on the table; `insert` on the others) primary. Other actions (refresh, edit, delete…) are `default`. Keys in `action-styles` are the action's exact internal `attr`. Custom action buttons resolve their style by `attr` too: a projected `o-table-button` resolves from its `o-table`, and a projected `o-button` resolves from the surrounding `o-table` / `o-grid` / `o-list` / `o-tree` / `o-form` (exposed through the new `OActionStyleProvider` DI token), so a custom button defaults to `outline` + `default` and automatically honours any `action-styles` keyed by its `attr` — explicit `variant`/`importance` on the button still win.
* **o-button**: New inputs `variant` (`outline` | `flat` | `basic` | `raised` | `icon` | `fab` | `mini-fab`) and `importance` (`primary` | `warn` | `default`), aligning `o-button` with the same semantic API as the components' built-in actions. `variant` maps to the Material button directive (outline → stroked, flat → flat, basic → basic, raised → raised, icon → icon, fab → fab, mini-fab → mini-fab); `importance` colours the text and icon on the light/transparent shapes (`outline`, `basic`, `icon`, `raised`), and the container colour on the filled shapes (`flat`, `fab`, `mini-fab`) so the label stays legible.
* **action-hierarchy**: New application-wide configuration via the `O_ACTION_STYLES_CONFIG` injection token (with a `provideOActionStyles(config)` helper from the public API). `OActionStylesConfig` exposes `default` (baseline `OActionStyle` applied to every action, replacing `outline + default`) and `actions` (per-`attr` overrides applied app-wide), so an app can set default variants/importances once instead of repeating `action-styles` on each component. The `resolveActionStyle` util now folds it into a per-field precedence (highest first): instance `action-styles[attr]` → global `actions[attr]` → component auto-rules → global `default` → framework default (`outline + default`). Therefore a per-instance `action-styles` still wins for that instance, the global per-`attr` config can override even the automatic primary highlight of the create action, and the global `default` stays below the auto-rules. Resolved by the built-in actions of `o-form` / `o-table` / `o-grid` / `o-list` / `o-tree`, by `o-table-button`, and by `o-button` (including standalone usage outside any host).
* **o-list / o-grid**: The built-in toolbar buttons (insert, refresh, and delete on `o-list` — `o-grid` has no delete button) now honour the `variant` from `action-styles` — not only the `importance`. In text mode (`show-buttons-text`) the button renders with the matching Material directive (outline → stroked, flat → flat, basic → basic, raised → raised) via a per-variant `@switch`; the filled variants (`flat`) route the importance to the container colour while the text/outline/raised variants colour the label, mirroring `o-button`. The floating insert FAB also routes `primary` / `warn` importance to its container colour. A new `getActionImportanceClassForVariant(attr)` helper on `AbstractOServiceComponent` returns the importance class only for the text-coloured variants. (In icon-only mode the toolbar buttons stay `mat-icon-button`.)
* **o-form-toolbar**: The built-in toolbar buttons (undo, refresh, insert, edit, delete, save, cancel and the confirm button) now honour the `variant` from `action-styles` / the global config too — not only the `importance` — in text mode (`show-header-actions-text`), via a per-variant `@switch` (outline → stroked, flat → flat, basic → basic, raised → raised). Filled variants route the importance to the container colour; icon-only mode keeps `mat-icon-button`. The toolbar gained `getActionVariant` / `getActionColor` / `getActionImportanceClassForVariant` helpers mirroring `AbstractOServiceComponent`. With this, every built-in action across `o-form` / `o-table` / `o-grid` / `o-list` supports both `variant` and `importance` (`o-tree`'s built-in buttons still honour `importance` only).
* **action-hierarchy**: A `flat` action with `default` importance now renders as a neutral **dark** solid button (charcoal `#2C2A29` container with white label/icon by default) **kept dark in both light and dark themes**, instead of Material M3's default filled colour (which is the primary tone). The styling is applied through a dedicated `.o-action--filled-default` class and is recolourable via two CSS custom properties — `--o-action-filled-default-bg` and `--o-action-filled-default-fg` — set once at `:root` (they survive the dark theme, which re-emits the class under `.o-dark` with higher specificity; a direct `--mdc-filled-button-container-color` override would not). Only `flat` is neutralised: `fab` / `mini-fab` variants keep Material's default container, and the built-in floating insert FAB of `o-list` / `o-grid` keeps Material's default background too (its icon is still tinted by the action importance).
* **inputs**: New `subscriptSizing` input on the input components (defined on `OFormDataComponent`, so available on `o-text-input`, `o-integer-input`, `o-real-input`, `o-date-input`, `o-combo`, `o-listpicker`, `o-currency-input`, `o-password-input`, `o-textarea-input`, etc.) and a matching `subscriptSizing` key in `OInputsOptions`. It maps to Angular Material's form-field `subscriptSizing`: `'fixed'` always reserves the vertical space for the hint/error line (default), while `'dynamic'` only occupies that space when a message is actually shown (useful for denser layouts). Resolution precedence: the per-input `subscriptSizing` value → `oInputsOptions.subscriptSizing` (e.g. provided globally) → default, which is `'dynamic'` when the input's `error-options` type is `lite` and `'fixed'` otherwise.

### Bug Fixes
* **o-form-toolbar**: Fixed a malformed CSS class on the icon-only edit button (`o-form-toolbar-button-o-form-toolbar-button-edit` → `o-form-toolbar-button o-form-toolbar-button-edit`).

### Deprecations
* **o-button**: Inputs `type` and `color` are deprecated in favour of `variant` and `importance` respectively. They remain fully functional for backwards compatibility; when both are set, the new inputs win. Every legacy `type` now has a `variant` equivalent (including `icon` → `icon`, `fab` → `fab`, `mini-fab` → `mini-fab`); `color="accent"` has no `importance` equivalent and keeps working via the legacy input.

---

## 18.0.0-next.6 (2026-06-01)
### Features
* **o-form-service**: New input `initial-filter-function` on `OFormServiceComponent` (and all components that extend it: `o-combo`, `o-listpicker`, `o-radio`, etc.). Accepts a callback `() => { [key: string]: any }` whose return value is merged into the query filter on every data request, after parent-keys resolution. Mirrors the existing `initial-filter-function` already present on `AbstractOServiceComponent` (table, list, grid).

### Bug Fixes
* **o-form**: `CanDeactivateFormGuard` is now provided in `OFormComponent`'s own `providers` array. Previously it was only declared in the deprecated `OFormModule`, causing a `NullInjectorError` when the standalone component is used without that module.
* **o-date-input**: `MAT_DATE_FORMATS` is now provided in `ODateInputComponent`'s own `providers` alongside `DateAdapter`. `MatDatepickerInput` requires both tokens; omitting `MAT_DATE_FORMATS` threw `createMissingDateImplError` when `o-date-input` was used standalone without `OCustomMaterialModule`.
* **o-table**: `OTableFilterByColumnService` is now provided in `OTableFilterByColumnDataDialogComponent`'s own `providers`. The service was only declared in `OTableComponent`, but `MatDialog` creates dialogs under the application-level injector, so it was not visible inside the dialog and caused a `NullInjectorError`.

---

## 18.0.0-next.5 (2026-05-27)
### Features
* **o-form**: New outputs `onInsertError`, `onUpdateError`, `onDeleteError` emitted when the corresponding persistence operation fails (complement to the existing `onInsert` / `onUpdate` / `onDelete` which fire on success). Each event emits the raw error value (`any`). The framework's default error dialog acts as a **fallback**: it is only shown when no listener is subscribed to the corresponding output (`EventEmitter.observed === false`), preserving existing 15.x behaviour for forms that do not handle these events.
* **o-form**: New inputs `insert-function`, `update-function`, `delete-function` and `query-function` on `OFormComponent` that override the data-service call for each persistence/read operation. When the corresponding function is provided, the form invokes it instead of `dataService[insertMethod|updateMethod|deleteMethod|queryMethod]`; when absent, behaviour is unchanged. Each function must return `Observable<ServiceResponse>`, so the form's existing pipeline (validation, `onBefore*` events, loader, response unwrapping, `postCorrect*`/`postIncorrect*`, form cache and `closeDetail`) keeps working untouched. Signatures mirror the data-service methods so they are drop-in: `insertFunction(values, entity, sqlTypes?)`, `updateFunction(filter, values, entity, sqlTypes?)`, `deleteFunction(filter, entity, sqlTypes?)`, `queryFunction(queryParams: OQueryParams)`. This enables use cases such as uploading files (e.g. `forkJoin([...uploads])`) before persisting and injecting the resulting codes into the record, while the form still drives validation and post-processing. Provide the functions via property binding (`[insert-function]="fn"`) or by assigning them on the component instance (`@ViewChild(OFormComponent)`). Notes: `query-function` bypasses the service's response parser / name-convention, so the emitted `ServiceResponse.data` must already match the form field names; errors thrown by a custom function reach `postIncorrect*` (and the `onInsertError`/`onUpdateError`/`onDeleteError` outputs) as whatever the returned `Observable` errors with (typically an `HttpErrorResponse`).
* **o-form**: New input `show-back-button` on `OFormComponent` that controls the visibility of the toolbar back-arrow button. Accepted values: `'auto'` (default — visible when the form is rendered as a detail, matching previous behaviour), `'yes'` / `'true'` / `'all'` (always visible), `'no'` / `'false'` (never visible), or a list of mode codes separated by `;` reusing the `header-actions` vocabulary: `R` (INITIAL mode), `I` (INSERT mode), `U` (UPDATE mode). Examples: `show-back-button="I;U"` (visible only while editing/creating), `show-back-button="R"` (visible only while viewing a record). The input is propagated from `OFormComponent` to `OFormToolbarComponent`; the existing `isDetail` check is preserved as the fallback when the input is not set, so all current forms continue to render the back button exactly as before.
...
* **o-table**: new input `initial-filter-function` , allowing a callback `() => Expression | { [key: string]: any }` that returns an initial filter to be applied on **every query**.([58770bc](https://github.com/OntimizeWeb/ontimize-web-ngx/commit/58770bc)) Closes [#2073](https://github.com/OntimizeWeb/ontimize-web-ngx/issues/2073)
