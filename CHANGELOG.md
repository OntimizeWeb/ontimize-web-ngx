## 18.0.0-next.14
### Bug Fixes
* **o-data-toolbar**: Now wraps onto extra lines (`flex-wrap: wrap`) instead of overflowing a single row when its content — title, quickfilter, buttons, menu — doesn't fit. `o-table`/`o-list`/`o-grid`'s toolbar (`.o-data-toolbar`, `display: flex; flex-direction: row`) had no `flex-wrap`, so on narrow screens the excess just overflowed the toolbar's own box; if an ancestor happened to clip overflow (e.g. an app wrapping the table in a rounded card via `overflow: hidden`), part of the toolbar — potentially an entire button or the options menu — became invisible and unreachable, with no scrollbar to get to it either.
